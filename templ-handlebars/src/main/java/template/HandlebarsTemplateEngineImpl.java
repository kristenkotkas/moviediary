...
public class HandlebarsTemplateEngineImpl extends CachingTemplateEngine<Template> implements HandlebarsTemplateEngine {
  private final static Path LOCAL = Paths.get(System.getProperty("user.dir"));
  ...
  protected HandlebarsTemplateEngineImpl(Path dir) throws NoSuchMethodException {
    ...
    this.templatesDir = dir != null && !dir.isAbsolute() ? LOCAL.resolve(dir) : dir;
  }
  ...
  public <S extends TypeSafeTemplate> S getSafeTemplate(RoutingContext ctx, String fileName, Class<S> type) {
    try {
      fileName = templatesDir != null ? templatesDir.resolve(fileName).toString() : fileName;
      Template template = templatesDir == null ? cache.get(fileName) : null;
      if (template == null) {
        synchronized (this) {
  ...
  private Object createTypeSafeTemplate(Template template, Class<?> type, RoutingContext ctx) {
    ...
    return Proxy.newProxyInstance(template.getClass().getClassLoader(), new Class[]{type}, (proxy, method, args) -> {
      if (method.isDefault()) {
