...
public class LocalDatabase {
    ...
    private static final String SQL_INSERT_MOVIES = "INSERT INTO Movies (Id, Title, Year, Image) " +
    ...
    public static Future<LocalDatabase> initializeDatabase(Vertx vertx, JsonObject config) {
        return future(fut -> ifPresent(new LocalDatabase(vertx, config
                ...
                .put("max_pool_size", 30)), db -> db.createTables()
                .doOnError(fut::fail)
                .map(res -> db.initialize().rxSetHandler())
    ...
    private Single<UpdateResult> createTables() {
    ...
    public Future<Void> initialize() {
        return future(fut -> client.rxGetConnection()
                .flatMap(conn -> conn.rxUpdate(SQL_INSERT_FORM_USER)
                        ...
                        .flatMap(res -> conn.rxUpdate(SQL_INSERT_MOVIES))
    ...
    private void test() throws SQLException {
        ...
        conn.prepareStatement(SQL_CREATE_WISHLIST);
        conn.prepareStatement(SQL_INSERT_FORM_USER);
        conn.prepareStatement(SQL_INSERT_FORM_SETTING);
        conn.prepareStatement(SQL_INSERT_FB_USER);
        conn.prepareStatement(SQL_INSERT_FB_SETTING);
        conn.prepareStatement(SQL_INSERT_MOVIES);
        conn.prepareStatement(SQL_INSERT_WISHLIST);
        conn.prepareStatement(SQL_TRUNCATE_TABLES);
    }
}
