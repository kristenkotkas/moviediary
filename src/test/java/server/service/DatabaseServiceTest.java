...
public class DatabaseServiceTest {
  ...
  private static server.service.rxjava.DatabaseService database;
  ...
  public static void setUp(TestContext ctx) throws Exception {
    ...
    initializeDatabase(vertx, config.getJsonObject("mysql")).rxSetHandler()
        ...
        .doOnError(ctx::fail)
        .flatMap(db -> Observable.just(DatabaseService.create(vertx.getDelegate(), config)).toSingle())
        .doOnSuccess(db -> database = new server.service.rxjava.DatabaseService(db))
        .doOnError(ctx::fail)
        .flatMap(db -> CommonUtils.<String>single(h -> vertx.getDelegate().deployVerticle(new ServerVerticle()
            .setDatabase(database.getDelegate()), new DeploymentOptions().setConfig(config), h)))
        .test()
