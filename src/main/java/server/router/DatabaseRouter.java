...
public class DatabaseRouter extends EventBusRoutable {
  ...
  public DatabaseRouter(Vertx vertx, SecurityConfig securityConfig, DatabaseService database, MailService mail) {
    super(vertx);
    ...
    listen(CHANGE_SERIES_INACTIVE,
           reply((username, seriesId) -> database.toggleSeriesState(username, seriesId, false)));
    listen(GET_INACTIVE_SERIES, reply((user, param) -> database.getInactiveSeries(user)));
    listen(CHANGE_SERIES_ACTIVE,
           reply((username, seriesId) -> database.toggleSeriesState(username, seriesId, true)));
    ...
    return (user, json) -> {
      ...
      json.getJsonArray("rows").stream()
          ...
          .forEach(jsonObj -> jsonObj
              .put("WasFirst", getFirstSeen(((byte) jsonObj.getValue("WasFirst") == 1)))
              .put("WasCinema", getCinema(((byte) jsonObj.getValue("WasCinema") == 1)))
  ...
  private void handleUsersFormInsert(RoutingContext ctx) {
    ...
    Predicate<JsonObject> userMatches = json -> json.getString(Column.USERNAME.getName()).equals(username);
    database.getUser(username).setHandler(resultHandler(ctx, result -> check(getRows(result)
                                                                                 .stream()
                                                                                 .map(obj -> (JsonObject) obj)
                                                                                 .noneMatch(userMatches), () -> {
