...
import static omdb.OmdbServiceImpl.Cache.AWARD;

/**
 * The Open Movie Database service implementation.
 */
class OmdbServiceImpl extends CachingService<JsonObject> implements OmdbService {
  ...
  protected OmdbServiceImpl(Vertx vertx, JsonObject config, io.vertx.ext.web.client.WebClient webClient) {
    super();
    this.vertx = vertx;
  ...
  private void get(String uri, CacheItem<JsonObject> cache, Future<JsonObject> future, Retryable retryable) {
    client.get(HTTPS, ENDPOINT, apikey + uri)
        ...
        .send(ar -> {
          if (ar.succeeded()) {
            if (ar.result().statusCode() == Status.OK) {
