...
import static util.TestingUtils.*;
...
public class DatabaseRouterTest {
  ...
  // TODO: 25/06/2017 http api for database
  // TODO: 25/06/2017 test that api locally, without auth
  // FIXME: 25/06/2017 rewrite

  @BeforeClass
  public static void setUpClass(TestContext ctx) throws Exception {
    vertx = Vertx.vertx();
    config = toTestingConfig(getConfig().put(HTTP_PORT, PORT));
    ...
    initializeDatabase(vertx, config).rxSetHandler()
        ...
        .flatMap(db -> vertx.rxDeployVerticle("database.DatabaseVerticle", new DeploymentOptions()
            .setConfig(config)))
        .test()
  ...
  public void testGetHistory(TestContext ctx) throws Exception {
    ...
    Async async = ctx.async();
    doRequest(client.get("/private/api/v1/history"), ctx, cookies, new JsonObject()
        .put("is-first", false)
        .put("is-cinema", false)
        .put("start", "1 January, 2017")
        .put("end", "31 December, 2017")
        .put("page", 0).encode())
        .doOnNext(res -> assertThat(res.statusCode(), Matchers.is(Status.OK)))
        .flatMap(TestingUtils::bodyToObservable)
